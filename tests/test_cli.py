...
class CliTests(unittest.TestCase):
    ...
    def _invoke_agent(self, args):
        ...
        with patch.object(cli_module.os, "execvpe", side_effect=fake_execvpe):
            result = self.runner.invoke(cli_module.main, args)
    ...
    def test_init_default_creates_environment_and_state(self):
        ...
        self.assertTrue((project_dir / ".cl9" / "claude").is_dir())
        self.assertIn("This is a cl9-managed project.", (project_dir / ".cl9" / "claude" / "CLAUDE.md").read_text())
        state = self._read_state(project_dir)
        self.assertEqual(state["type"], "default")
        self.assertEqual(
            set(state["files"]),
            {"README.md", "MEMORY.md", "flake.nix", ".envrc", ".cl9/claude/CLAUDE.md"},
        )
    ...
    def test_init_minimal_creates_only_cl9_state(self):
        ...
        self.assertTrue((project_dir / ".cl9" / "claude" / "CLAUDE.md").exists())
        self.assertFalse((project_dir / "README.md").exists())
        self.assertFalse((project_dir / "src").exists())
        self.assertEqual(
            self._read_state(project_dir)["files"],
            {".cl9/claude/CLAUDE.md": self._read_state(project_dir)["files"][".cl9/claude/CLAUDE.md"]},
        )
    ...
    def test_init_is_noop_for_existing_project(self):
    ...
    def test_agent_spawn_finds_project_root_from_subdirectory(self):
        ...
        self.assertIn("Spawning agent in project: agent-project", result.output)
        self.assertEqual(captured["executable"], "claude")
        self.assertEqual(
            captured["argv"],
            [
                "claude",
                ...
                str((project_dir / ".cl9" / "claude" / "CLAUDE.md").resolve()),
                "--continue",
            ],
        )
        self.assertEqual(captured["cwd"], str(nested_dir.resolve()))
        self.assertNotIn("CLAUDE_CONFIG_DIR", captured["env"])
    ...
    def test_agent_spawn_no_continue_omits_continue_flag(self):
        project_dir = self.work_dir / "agent-nocontinue"
        project_dir.mkdir()
        self.runner.invoke(cli_module.main, ["init", str(project_dir), "--type", "minimal"])
        self._chdir(project_dir)

        _, captured = self._invoke_agent(["agent", "spawn", "--no-continue"])
    ...
    def test_agent_spawn_uses_project_local_settings_and_mcp_overrides(self):
        ...
        project_dir.mkdir()
        self.runner.invoke(cli_module.main, ["init", str(project_dir), "--type", "minimal"])
        claude_dir = project_dir / ".cl9" / "claude"
        ...
        self.assertEqual(
            ...
            [
                ...
                "user",
                "--append-system-prompt-file",
                str((claude_dir / "CLAUDE.md").resolve()),
                "--settings",
                str((claude_dir / "settings.json").resolve()),
                "--mcp-config",
                str((claude_dir / "mcp.json").resolve()),
                "--continue",
            ],
        )
    ...
    def test_update_updates_unchanged_files(self):
        ...
        self._chdir(project_dir)
        result = self.runner.invoke(cli_module.main, ["update"])
    ...
    def test_update_skips_modified_files(self):
        template_dir = self._create_template({"README.md": "# one\n"})
    ...
    def test_update_force_overwrites_modified_files(self):
        template_dir = self._create_template({"README.md": "# one\n"})
        ...
        (template_dir / "README.md").write_text("# two\n")
        self._chdir(project_dir)
        result = self.runner.invoke(cli_module.main, ["update", "--force"])

        self.assertEqual(result.exit_code, 0)
        self.assertIn("Updated:     README.md (was modified)", result.output)
        self.assertEqual((project_dir / "README.md").read_text(), "# two\n")

    def test_update_diff_does_not_modify_files(self):
        template_dir = self._create_template({"README.md": "# one\n"})
    ...
    def test_update_requires_state_tracking(self):
